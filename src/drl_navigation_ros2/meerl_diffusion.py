...
class Diffusion(nn.Module):
    ...
    def ddim_sample_loop(self, state, shape, stride, K_repeat=1):
        ...
        for i in reversed(range(0, self.n_timesteps, stride)):
            ...
            if self.predict_epsilon:
                x_i = alpha_bar_prev.sqrt() * (x_i - (1 - alpha_bar).sqrt() * noise_x) / alpha_bar.sqrt() + (1 - alpha_bar_prev).sqrt() * noise_x
            else:
    ...
    def get_loss(self, x_start, state, weight=1.0):
        ...
        loss_mean = loss.mean(dim=1, keepdim=True)
        wloss = loss_mean * weight
        return wloss.mean()
...
def main():
    ...
    # 准备输入数据
    batch_size = 2
    state = torch.randn(batch_size, 5).to(device)  # 将state移到正确的设备上
